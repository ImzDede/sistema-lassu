...
const Calendario = () => {
  ...
  // MOCK DE AGENDAMENTOS (DADOS FALSOS)
        ...
        {diasDaSemana.map((dia, index) => {
          ...
          return (
            <div 
              ...
              `}
            >
              <span className={`font-bold text-lg mb-2 ${ehHoje ? 'text-blue-600' : 'text-black'}`}>
                {format(dia, 'd')}
              </span>
              ...
              {temAgendamento.length > 0 && (
                <div className="flex flex-col gap-1 w-full px-1">
                  {temAgendamento.map((_, i) => (
                     <div key={i} className="h-8 w-full bg-gray-500 border border-gray-600"></div>
                  ))}
                </div>
              )}
