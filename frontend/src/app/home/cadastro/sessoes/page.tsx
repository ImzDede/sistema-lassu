...
export default function RegisterSession() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showFeedback("Sessão cadastrada!", "success");
      setTimeout(() => router.push("/home"), 1500);
    } catch (error: any) {
        showFeedback(error.message || "Erro ao criar sessão.", "error");
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full pb-10">
      <FeedbackAlert open={feedback.open} color={feedback.type === "error" ? "red" : "green"} message={feedback.message} onClose={closeFeedback} />
      ...
      <Card className="w-full shadow-lg border-t-4 border-brand-purple bg-brand-surface">
        <CardBody className="p-6 md:p-10">
          <div className="flex items-center gap-3 mb-8 pb-4 border-b border-gray-100">
            <div className="p-2 bg-brand-purple/10 rounded-lg"><Users className="w-6 h-6 text-brand-purple" /></div>
